...
//        AES_ctr128_encrypt(data.data(), ciphertext.data() + size_vec_size, (unsigned long) data.size(), &encryption_key, iv.data(), counter, &num, AES_ENCRYPT);
        ...
        int length;
        if (1 != EVP_EncryptUpdate(ctx, ciphertext.data(), &length, data.data(), data.size()))
        {
            // TODO: Throw Exception
        }

        // ciphertext.data() + length
        if (1 != EVP_EncryptFinal_ex(ctx, ciphertext.data(), &length))
        {
